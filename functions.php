...
function header_images() {
	...
	if ( is_singular() && ( ! empty( $custom_image ) || has_post_thumbnail() ) ) {
		if ( $custom_image )
			echo '<img src="' . esc_url( str_replace('.jpg','-1500x697.jpg',$custom_image) ) . '" alt="" class="header-img" />';
		else
			the_post_thumbnail( 'smamo-header-img', array( 'class' => 'header-img' ) );
	} else {
...
/* SMARTMONKEY INCLUDES STUFF HERE */

include 'library/smamo-ct.php'; // Kontaktformular

include 'smamo-logo-options.php'; // Logo

add_image_size( 'smamo-header-img', 1500, 697, true );
...
/* Opret ekstra post type til citater
/* ------------------------------------- */


add_action( 'init', 'smamo_post_types' );
function smamo_post_types() {
	...
	$labels = array(
		'name'               => _x( 'Feedback', 'post type general name', 'smamo' ),
		'singular_name'      => _x( 'Feedback', 'post type singular name', 'smamo'),
		'menu_name'          => _x( 'Feedback', 'admin menu', 'smamo'),
		'name_admin_bar'     => _x( 'Feedback', 'add new on admin bar', 'smamo'),
		'add_new'            => _x( 'Tilføj ny', 'feedback', 'smamo'),
		'add_new_item'       => __( 'Tilføj ny feedback', 'smamo'),
		'new_item'           => __( 'Ny feedback', 'smamo'),
		'edit_item'          => __( 'Rediger feedback', 'smamo'),
		'view_item'          => __( 'Se feedback', 'smamo'),
		'all_items'          => __( 'Alle', 'smamo'),
		'search_items'       => __( 'Søg i Feedback', 'smamo'),
		'parent_item_colon'  => __( 'Forælder:', 'smamo'),
		'not_found'          => __( 'ingen Feedback fundet :-(', 'smamo'),
...
// Citat-widget
class smamo_feed_box extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        wp_enqueue_style('smamo-feedback-style', get_template_directory_uri().'/library/smamo-feed/style.css', '', false, 'all' );
        wp_enqueue_script('smamo-feedback-script',  get_template_directory_uri().'/library/smamo-feed/script.js', array('jquery'), '1.0', true);
        
        echo $args['before_widget']; 
        
        // Render
        echo '<div id="smamo-feedback-loop">';
            
        $smamo_q = get_posts(array(
            'post_type'         => 'feedback',
            'numberposts'       => -1,
            'orderby'          => 'rand' 
        ));
        
        foreach ($smamo_q as $feedback):
            
        $feed_quote = get_post_meta($feedback->ID,'feed_quote',true);
        $feed_name = get_post_meta($feedback->ID,'feed_name',true);
        
        ?>
            <div class="smamo_feedback" id="feed_<?php echo $feedback->ID ?>">
                <span class="feed_quote"><?php echo apply_filters('the_excerpt',$feed_quote); ?></span>
                <span class="feed_name"><?php echo esc_html($feed_name) ?></span>
            </div>
    ...
    public function form( $instance ) {
        ...
        // Widget admin form (ingen felter)
        ?><p>Viser en rotation af feedback</p><?php 
    }
...
function smamo_load_widgets() {
	register_widget( 'smamo_feed_box' );
}
